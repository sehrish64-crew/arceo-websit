...
export default function ClinicModal({ isOpen2, onClose }: ClinicModalProps) {
    const [ModalAnimation2, setModalAnimation2] = useState(false);
    const [selectedTreatment, setSelectedTreatment] = useState(treatments[0]);
    // Effect to handle modal animation when isOpen changes
    useEffect(() => {
        if (isOpen2) {
            setTimeout(() => setModalAnimation2(true), 10);
        } else {
            setModalAnimation2(false);
        }
    }, [isOpen2]);
    ...
    return (
        ...
        >
           ...
           sm:align-middle w-full max-w-4xl mx-auto
                     ${ModalAnimation2 ? "scale-100 skew-y-0" : "scale-0 skew-y-6"}
